defmodule DsaWeb.CharacterBlessingController do
  @moduledoc """
  Handles all actions related to a user's character blessings.
  """
  ...
  def update_all(conn, %{"character" => params}, character) do
    case Characters.update(character, params) do
      {:ok, character} ->
        ...
        |> redirect(to: Routes.character_blessing_path(conn, :index, character))
  ...
  def add_all(conn, _params, character) do
    case Characters.add_blessings(character) do
      {:ok, character} ->
        conn
        |> put_flash(:info, gettext("Blessings updated successfully."))
        |> redirect(to: Routes.character_blessing_path(conn, :edit_all, character))
  ...
  def remove_all(conn, _params, character) do
    case Characters.remove_blessings(character) do
      {:ok, character} ->
        conn
        |> put_flash(:info, gettext("Blessings updated successfully."))
        |> redirect(to: Routes.character_blessing_path(conn, :edit_all, character))
