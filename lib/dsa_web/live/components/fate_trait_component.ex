defmodule FateTraitComponent do
  use DsaWeb, :live_component

  import DsaWeb.CharacterHelpers, only: [ap: 2]

  alias Dsa.Data.FateTrait

  @changeset_type :fate_traits
  ...
  ~L"""
    <div class="card px-0 my-2">
      <header class="card-header">
        ...
        <p class='card-header-title pl-0 py-0'>Schicksalspunkte Sonderf.</p>
        <span class='tag is-light my-1 mr-1'>
          <strong><%= ap(@character, :fate_traits) %></strong>
          &nbsp;AP
        </span>
      </header>
      <div class="card-content px-1 py-2">
        <%=
          @character.fate_traits
          |> Enum.map(& FateTrait.name(&1.fate_trait_id))
          |> Enum.join(", ")
        %>
        <%= if Enum.count(@character.fate_traits) == 0 do %>
          <%= @character.name %> hat keine Schicksalspunkte Sonderf.
        <% end %>
      </div>
    </div>
  """
  ...
  def render(assigns) do
    ~L"""
      <div class="card px-0 my-2">
        <header class="card-header">
          <p class='card-header-title pl-2 py-0'>Schicksalspunkte Sonderf.</p>
          <form phx-change='add' phx-target='<%= @myself %>'>
            <%= Phoenix.HTML.Form.select :entry, :id, FateTrait.options(@changeset),
        ...
        <div class='card-content px-0 pt-2 pb-0'>
          ...
          <%= hidden_inputs_for(f) %>
            <table class='table is-size-7 is-fullwidth mb-1'>
              ...
              <tbody>
                <%= for fs <- inputs_for(f, :fate_traits) do %>
                  <%
                    id = input_value(fs, :fate_trait_id)
                    id = if is_integer(id), do: id, else: String.to_integer(id)
                  %>
                  <tr>
                    <%= hidden_inputs_for(fs) %>
                    <td><%= FateTrait.name(id) %></td>
                    <td class='has-text-centered'><%= FateTrait.ap(id) %></td>
